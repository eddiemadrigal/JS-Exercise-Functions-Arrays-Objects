...
function getCarInfoById(inventory, carID) {
  const carInfo = inventory.find(({ id }) => {
    return id === carID;
  });
  return `This is a ${carInfo.car_make} ${carInfo.car_model}`;
}

//console.log(getCarInfoById(dataInventory, 2));
...
function sortCarInventory(inventory) {
  inventory.sort(function(a, b) {
    let car_modelA = a.car_model.toUpperCase();
    let car_modelB = b.car_model.toUpperCase();
    if (car_modelA < car_modelB) return -1;
...
function getModelYears(carInventory) {
  let myArr = [];
  for (let i = 0; i < carInventory.length - 1; i++) {
    myArr.push(carInventory[i].car_year);
  }
  return myArr;
}
...
function carMaker(odometer) {
  const objResult = {
    ...
    drive: function(distance) {
      let updatedOdommeter = odometer + distance;
      return updatedOdommeter;
    }
  }
  return objResult;
}
